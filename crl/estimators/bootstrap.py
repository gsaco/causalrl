...
def _resample_dataset(
    ...
    if isinstance(data, LoggedBanditDataset):
        ...
        return LoggedBanditDataset(
            ...
            behavior_action_probs=(
                data.behavior_action_probs[idx]
    ...
    if config.method == "iid":
        ...
        obs = flat_obs[idx].reshape(
            data.num_trajectories, data.horizon, *flat_obs.shape[1:]
        )
        next_obs = flat_next_obs[idx].reshape(
            data.num_trajectories, data.horizon, *flat_next_obs.shape[1:]
        )
        ...
        behavior_action_probs = (
            flat_probs[idx].reshape(data.num_trajectories, data.horizon)
    ...
    if config.method == "block":
        ...
        behavior_action_probs_src = data.behavior_action_probs
        behavior_action_probs = (
            np.zeros_like(behavior_action_probs_src)
        ...
        for i in range(data.num_trajectories):
            ...
            while pos < data.horizon:
                traj_idx = int(rng.integers(0, data.num_trajectories))
                start = int(
                    rng.integers(0, max(1, data.horizon - config.block_size + 1))
                )
                end = int(min(start + config.block_size, data.horizon))
                length = int(end - start)
                obs[i, pos : pos + length] = data.observations[traj_idx, start:end]
                next_obs[i, pos : pos + length] = data.next_observations[
                    traj_idx, start:end
                ]
                ...
                if behavior_action_probs is not None:
                    ...
                    behavior_action_probs[i, pos : pos + length] = (
                        behavior_action_probs_src[traj_idx, start:end]
                    )
                pos += length
        return TrajectoryDataset(
            observations=obs,
