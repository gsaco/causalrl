"""Generalized DICE estimator for density ratio OPE."""
...
class GenDICEEstimator(OPEEstimator):
    ...
    def __init__(
        ...
        config: GenDICEConfig | None = None,
        ...
        self.config = config or GenDICEConfig()
    ...
    def estimate(self, data: TrajectoryDataset) -> EstimatorReport:
        ...
        if data.state_space_n is None:
            raise ValueError("GenDICE requires discrete state_space_n.")
        ...
        if self.config.alpha != 1.0:
            density_ratio = np.power(density_ratio, self.config.alpha)
        ...
        return self._build_report(
            ...
            warnings=warnings,
            metadata={"estimator": "GenDICE", "config": self.config.__dict__},
            data=data,
        )
...
__all__ = ["GenDICEConfig", "GenDICEEstimator"]
